 ...
 */
export const authenticate = (tokenType = TokenTypeEnum.Access) => {
    ...
    return async (req, res, next) => {
        try {
            ...
            if (!allowedRoles.includes(req.user.role)) {
                throw new ForbidenException(`Access denied. Required roles: ${allowedRoles.join(', ')}`);
            }
            next();
        }
        catch (error) {
 ...
 */
export const checkGovernorateAccess = (governorateIdParam = 'governorateId') => {
    return async (req, res, next) => {
        try {
            // Check if user is authenticated
            if (!req.user) {
            ...
            if (userGovernorateId !== targetGovernorateId) {
                throw new ForbidenException("You can only access data from your assigned governorate");
            }
            next();
        }
        catch (error) {
    ...
    return async (req, res, next) => {
        try {
            ...
            if (resource.createdBy.toString() !== req.user._id.toString()) {
                throw new ForbidenException("You can only modify resources you created");
            }
 ...
 */
export const optionalAuthenticate = (tokenType = TokenTypeEnum.Access) => {
    return async (req, res, next) => {
        try {
